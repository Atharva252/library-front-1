...
import { Upload, BookOpen, Image, FileText, User, Tag, MessageSquare, Check, AlertCircle, Star, Globe, Shield, Zap, Crown, Sparkles, ChevronDown, Loader, Wand2, RefreshCw } from 'lucide-react';
import { watermarkRemover } from '../utils/watermarkRemover';

const API_BASE_URL = 'http://localhost:5000/api';

export default function UpdatedBookUpload() {
  ...
  // Extract PDF first page as cover with watermark removal
  const extractPDFCover = async (file) => {
    try {
      console.log('📖 Extracting PDF first page as cover with watermark removal...');
      ...
      console.log('✅ PDF cover extracted with watermark removal');
      return coverImage;
      
    } catch (error) {
      console.error('❌ PDF cover extraction failed:', error);
      const cleanTitle = watermarkRemover.cleanFilename(file.name);
      return watermarkRemover.generateCleanFallbackCover(cleanTitle);
    }
  };

  // Generate fallback cover
  const generateFallbackCover = (title) => {
    ...
    ctx.fillText('PDF Cover', 150, 180);
    ctx.fillText('Not Available', 150, 210);
  ...
  const extractPDFData = async (file) => {
    try {
      ...
      // Extract cover with watermark removal
      ...
      try {
        ...
        const arrayBuffer = await file.arrayBuffer();
        const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        metadata = await watermarkRemover.extractCleanMetadata(pdf);
  ...
  // Regenerate PDF cover
  const regeneratePDFCover = async () => {
    if (!pdfFile) return;
    
    setExtracting(true);
    try {
      const coverImage = await extractPDFCover(pdfFile);
      setFormData(prev => ({
        ...prev,
        coverImage: coverImage
      }));
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              Smart PDF processing with watermark removal, author extraction, and metadata detection
            </p>
          </div>
                ...
                <div className="bg-white/90 backdrop-blur-sm px-8 py-6 rounded-t-3xl">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h2 className="text-2xl font-bold text-slate-700">PDF Cover Extractor</h2>
                        <p className="text-teal-600">Automatically extracts PDF first page as cover</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2 bg-emerald-100 px-4 py-2 rounded-full">
                      <Shield className="h-4 w-4 text-emerald-600" />
                      <span className="text-sm text-emerald-700 font-medium">Smart Upload</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="xl:col-span-1 space-y-8">
                          ...
                          {imagePreview ? (
                            ...
                            <div className="w-full aspect-[3/4] bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl flex items-center justify-center border-2 border-dashed border-gray-300">
                              <div className="text-center">
                                ...
                                <p className="text-gray-500 text-sm">Upload PDF to extract first page</p>
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border border-emerald-200/50">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                          <span className="text-gray-600 text-sm">Smart title & metadata extraction</span>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          {extracting ? (
                            <div className="space-y-3">
                              <div className="flex items-center justify-center space-x-2">
                                ...
                                <p className="text-teal-700 font-semibold text-lg">Extracting PDF Cover...</p>
                              </div>
                              <div className="bg-teal-50 rounded-lg p-4 max-w-md mx-auto border border-teal-200">
                                <p className="text-slate-700 text-sm">
                                  ✨ Extracting first page as cover image
                                </p>
                              </div>
                            </div>
                          ) : formData.pdfFile ? (
                            <div className="space-y-3">
                              ...
                              <div className="bg-emerald-50 rounded-lg p-4 max-w-md mx-auto border border-emerald-200">
                                ...
                                <p className="text-gray-600 text-sm">✅ Cover extracted from first page</p>
                              </div>
                            </div>
                          ) : (
                            <div>
                              <p className="text-slate-700 font-semibold text-xl mb-2">Drop your PDF here</p>
                              <p className="text-gray-600 mb-4">First page will be extracted as cover!</p>
                              <div className="flex items-center justify-center space-x-4 text-sm text-gray-500 mb-4">
                                <span>• Max size: 50MB</span>
                                <span>• PDF format only</span>
                                <span>• Auto cover extraction</span>
                              </div>
                              <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
                                <p className="text-xs text-teal-700">
                                  🚀 <strong>Smart Upload:</strong> PDF first page will be automatically used as cover!
                                </p>
                              </div>
                            </div>
                          )}
                    ...
                    >
                      {extracting ? (
                        <>
                          ...
                          <span>Extracting PDF Cover...</span>
                        </>
                      ) : uploading ? (
                        <>
                        ...
                        <>
                          ...
                          <span>Publish Book with PDF Cover</span>
                        </>
                      )}
                    </button>
                  </div>
